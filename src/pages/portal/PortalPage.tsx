...
export default function PortalPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      // Fetch active contract with property
      const { data: contractData } = await supabase
        .from('contracts')
        .select('id, start_date, end_date, current_rent, currency, is_active, properties(full_address)')
        .eq('is_active', true)
        .limit(1)
        .single()
      ...
      if (duesData) setRentDues(duesData)
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {rentDues.slice(0, 6).map(due => {
              ...
              return (
                <div key={due.id} className="flex items-center justify-between rounded-md border p-3">
                  <div>
